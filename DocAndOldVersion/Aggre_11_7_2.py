#coding:utf-8
"""version 11.7.2 :  1-优化  2-通过获取获取窗口变化大小，重绘页面组件。
...
class MyFrame(wx.Frame):
	...
	def __init__(self):
		...
		medviewFile=''
		def getWorkPathEdit():
			return locationText.GetValue()

		def setWorkPathEdit(argStr):
			locationText.SetValue(argStr)
		...
		def OpenTheLocation(event):    #数据结构[{'fileName':name,'isDir':True},{'fileName':name2,'isDir':False},...]
			...
			caculateButtons(ls,theWidth,theHeight)

		def caculateButtons(ls,theWidth,theHeight):
		...
		def OnMyResize(event):
			global size 
			size = event.Size
			#print size
			# global ls
			# if ls!=[]:
				#caculateButtons(ls,size.width,size.height)
		...
		def OnTimer(event):	
			...
			global ls
			if ls!=[]:
				caculateButtons(ls,size.width,size.height+40)
		...
		panel.Bind(wx.EVT_SIZE,OnMyResize)

		timer = wx.Timer(owner=None, id=-1)
		...
		loadButton = wx.Button(panel,label = u'打开工作位置', pos=(225,5),size=(130,25))
