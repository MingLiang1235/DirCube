#coding:utf-8
"""version 11.9 : 清理窗口内容 通过获取获取窗口变化大小，重绘页面组件。
...
class MyFrame(wx.Frame):
	...
	def __init__(self):
		...
		def OnMyResize(event):
			...
			timer.Start(milliseconds=300,oneShot=True)      #new Start erase old ones
			event.Skip()
		def OnTimer(event):	
			#print size
			global ls
			if ls!=[]:
				caculateButtons(ls,size.width,size.height+39)
		...
		width = 900
		height = 535
		posInit = 39	#原值为65：Y轴起始位置(工具栏的高度)
