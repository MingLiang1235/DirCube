...
class MyFrame(wx.Frame):
	...
	def __init__(self):
		...
		def OpenTheLocation(event):    #数据结构[{'fileName':name,'isDir':True},{'fileName':name2,'isDir':False},...]
			...
			consoleText.AppendText("the row of origin dir is :"+str(len(sizes))+'\n')
			for i in range(len(sizes)):
				consoleText.AppendText(sizes[i]['name']+"->"+str(sizes[i]['size'])+'\n')
			consoleText.AppendText("---------------\n")
			ls=getMaxs(sizes)
			for i in range(0,len(ls)):    #len(ls) == maxButtons refer to getMaxs(ls).
				consoleText.AppendText(ls[i]['name']+'->'+str(ls[i]['size'])+'\n')
				buttons[i].SetLabel(ls[i]['name']+' \n\t'+str(ls[i]['size']/1024.0/1024.0)+'M '+('isDir'if (ls[i]['isDir']) else 'isFile'))  #如果是File，则不可以goThrough下一步了，这个要处理。另外如果是File为何没有name和size值？
			consoleText.AppendText("===============\n")
		def getMaxs(ls):
		...
		wx.Frame.__init__(self,None,-1,u"TextTreater's 修改本  v.0.4",size=(900,535))
