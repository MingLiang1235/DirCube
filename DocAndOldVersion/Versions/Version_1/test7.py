...
class MyFrame(wx.Frame):
	...
	def __init__(self):
		...
		def OpenTheLocation(event):    #数据结构[{'fileName':name,'isDir':True},{'fileName':name2,'isDir':False},...]
			...
			ls = getMaxs(sizes)
			ls = DevideRec.devideRec(ls,width/2,height)
			...
			#vbox0.Clear()
			#panel.Refresh()
			for i in range(0,maxButtons):    #len(ls) == maxButtons refer to getMaxs(ls).
				if i < len(ls):
		...
		def generateButton(L):
			...
			for i in range(maxButtons):
				...
				tip.SetTip(buttons[i].GetLabel())
				consoleText.AppendText("str(button("+str(i)+")):" + str(i.pos) + "  "+str(i.size))
			consoleText.AppendText("\n---------------\n")
