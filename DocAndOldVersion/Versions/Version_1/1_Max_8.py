...
class MyFrame(wx.Frame):
	...
	def __init__(self):
		...
		def OpenTheLocation(event):    #数据结构[{'fileName':name,'isDir':True},{'fileName':name2,'isDir':False},...]
			...
			consoleText.AppendText("NOW OPEN THE LOC::::::::: \n")
			...
			ls = DevideRec.devideRec(ls,width/2,height-posInit-titleHeight)    # posInit  Y轴起始位置  titleHeight 标题栏的高度
			for i in range(len(ls)):
				consoleText.AppendText('L['+ str(i) +']: '+ str(ls[i])+"\n")
				consoleText.AppendText("---------------\n")
			consoleText.AppendText("in OpenLocation,before destroy Buttons is : "+str(buttons)+"\n")
		...
		def generateButton(L):
			...
			for i in range(len(L)):
				...
				consoleText.AppendText("str(button("+str(i)+")):" + str(position) + "  "+str(sizeofB))
			consoleText.AppendText("\n---------------\n")
