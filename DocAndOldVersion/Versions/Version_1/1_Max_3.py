...
class MyFrame(wx.Frame):
	...
	def __init__(self):
		...
		def OpenTheLocation(event):    #数据结构[{'fileName':name,'isDir':True},{'fileName':name2,'isDir':False},...]
			...
			for fname in fileNameList:
				...
				if S_ISDIR(pathStat.st_mode):
					...
					#dict_fileNamesOneLevel.append(entity_fileName)
				dictSize={'name':pathName,'size':size,'isDir':isDir}
				sizes.append(dictSize)
			ls=getMaxs(sizes)
			for i in range(0,len(ls)):
				consoleText.AppendText(ls[i]['name']+' '+str(ls[i]['size']))
			firstButton.SetLabel(ls[0]['name']+' \n\t'+str(ls[0]['size']/1024/1024)+'M '+'isDir'if (ls[0]['isDir']) else 'isFile')  #如果是File，则不可以goThrough下一步了，这个要处理。另外如果是File为何没有name和size值？
			if i > 0:
				secondButton.SetLabel(ls[1]['name']+' \n\t'+str(ls[1]['size']/1024/1024)+'M '+'isDir'if (ls[1]['isDir']) else 'isFile')
			if i > 1:
				thirdButton.SetLabel(ls[2]['name']+' \n\t'+str(ls[2]['size']/1024/1024)+'M '+'isDir'if (ls[2]['isDir']) else 'isFile')
		def getMaxs(ls):
		...
		def goThrough(event):
			global ls
			if ls: 
				if event.GetEventObject()==firstButton:
					locationText.SetValue(ls[0]['name'])
				if event.GetEventObject()==secondButton:
					locationText.SetValue(ls[1]['name'])
				if event.GetEventObject()==thirdButton:
					locationText.SetValue(ls[2]['name'])
